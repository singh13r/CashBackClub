        ...
        async function handleSignup() {
            ...
            pendingSignup = {
                ...
                phone: signupOtpMethod === "phone" ? contact.value : "",
                authValue: contact.authValue,
                method: signupOtpMethod
            };
            localStorage.setItem(PENDING_SIGNUP_KEY, JSON.stringify(pendingSignup));

            showToast(signupOtpMethod === "email" ? "Sending verification link..." : "Sending phone verification...");
            var payload = signupOtpMethod === "phone" ? {
                phone: contact.authValue
            } : {
                email: contact.authValue,
                options: {
                    emailRedirectTo: window.location.href.split("#")[0]
                }
            };
            var result = await supabase.auth.signInWithOtp(payload);
            if (result.error) {
                showToast("Error: " + result.error.message);
                return;
            }
            showToast(signupOtpMethod === "email" ? "Verification link sent. Check your email." : "Phone verification sent. Supabase phone signup needs SMS provider setup.");
        }
        ...
        function fetchLivePriceData(productId) {
            if (!pendingLivePriceRequests[productId]) {
                pendingLivePriceRequests[productId] = supabase.functions.invoke("live-price", {
                    body: {
                        productId: productId
                    }
        ...
        function renderComparePrices(p, compare, sourceLabel) {
            ...
            var savingsText = hasBothPrices ? gap === 0 ? "Both stores are currently at the same price." : cheaperMarket + " is cheaper by Rs." + gap.toLocaleString("en-IN") + " (" + gapPercent + "%) compared with " + expensiveMarket + "." : "Use the store buttons to confirm the current exact variant price.";
            document.getElementById("compareModalSub").textContent = "Showing compare prices and direct store links. Open the store to confirm the latest exact variant price.";
            document.getElementById("compareGapValue").textContent = hasBothPrices ? gap === 0 ? "0" : "Rs." + gap.toLocaleString("en-IN") : "Checking";
            document.getElementById("compareGapLabel").textContent = "Price Gap";
            ...
            document.getElementById("compareAmazonNote").textContent = !hasAmazonPrice ? "Check store" : hasBothPrices && amazonValue <= flipkartValue ? "Lower saved price" : hasBothPrices ? "Higher saved price" : "Saved price";
            document.getElementById("compareFlipkartNote").textContent = !hasFlipkartPrice ? "Check store" : hasBothPrices && flipkartValue <= amazonValue ? "Lower saved price" : hasBothPrices ? "Higher saved price" : "Saved price";
        }
        ...
        function openCompareModal(id, event) {
            ...
            renderComparePrices(p, compare, "manual");
        ...
        function renderProducts(filter) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                var hasFlexiblePriceLabel = /live|check store/i.test(String(p.price));
                var tag = hasFlexiblePriceLabel || getPrice(p.price) >= 1000 ?
