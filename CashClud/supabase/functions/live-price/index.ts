...
const PRODUCT_URLS: Record<string, { amazon: string; flipkart: string }> = {
  ...
  \"8\": {
    amazon: \"https://www.amazon.in/s?k=HP+Laptop+15s+Core+i5+13th+Gen\",
    flipkart: \"https://www.flipkart.com/search?q=HP+Laptop+15s+Core+i5+13th+Gen\",
  },
  \"9\": {
    amazon: \"https://www.amazon.in/s?k=Noise+ColorFit+Pulse+4+Smartwatch\",
    flipkart: \"https://www.flipkart.com/search?q=Noise+ColorFit+Pulse+4+Smartwatch\",
  },
  \"10\": {
...
const PRODUCT_VARIANT_HINTS: Record<string, string[]> = {
  ...
  "7": ["fr2 pro"],
  "8": ["hp 15s", "i5", "13th gen"],
  "9": ["pulse 4"],
  "10": ["echo dot", "5th gen"],
};

const PRICE_PATTERN = /(?:â‚¹|₹|Rs\.?|INR)\s*([1-9][0-9,]*(?:\.[0-9]{1,2})?)/gi;
const AMAZON_WHOLE_PATTERN = /a-price-whole[^>]*>\s*([0-9,]+)(?:\s*<span[^>]*>\.<\/span>)?/gi;
...
function normalizePriceFromHtml(html: string, market: "amazon" | "flipkart", limits: { min: number; max: number }, hints: string[]) {
  ...
  const strongPatterns = market === "amazon"
    ? [
        ...
        /id="priceblock_ourprice"[^>]*>\s*[^0-9]*([0-9,]+(?:\.[0-9]{1,2})?)/gi,
        /id="priceblock_dealprice"[^>]*>\s*[^0-9]*([0-9,]+(?:\.[0-9]{1,2})?)/gi,
        /class="a-offscreen"[^>]*>\s*[^0-9]*([0-9,]+(?:\.[0-9]{1,2})?)/gi,
      ]
    : [
        /"finalPrice"\s*:\s*\{"value"\s*:\s*([0-9]+(?:\.[0-9]{1,2})?)\}/gi,
        /"sellingPrice"\s*:\s*\{"amount"\s*:\s*([0-9]+(?:\.[0-9]{1,2})?)\}/gi,
        /class="_30jeq3 _16Jk6d"[^>]*>\s*[^0-9]*([0-9,]+(?:\.[0-9]{1,2})?)/gi,
        /class="_30jeq3"[^>]*>\s*[^0-9]*([0-9,]+(?:\.[0-9]{1,2})?)/gi,
